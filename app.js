...
app.intent('Vewd',
    {
        "slots":{},
        "utterances":[ "{go to|open|turn on|open up|display|jump to|launch|} {viewed|viewed app store}" ]
    },
    function(request,response) {
        if (checkAppEnabled(strings.vewd)) {
            response.say("Launching " + strings.vewd);
            var commands = [];
            commands = addInitCommands(commands);
            commands = openAppsCommands(commands);
            commands = buildAppNavigation(strings.vewd, commands);
            sendCommands(commands);
        }
        else {
            response.say(strings.vewd + strings.txt_notenabled);
        }
    });
...
function updateCurrentTiVoConfig(tivoIndex) {
    ...
    // update apps status
    apps_status = [config.tivos[tivoIndex].netflix, config.tivos[tivoIndex].amazon, config.tivos[tivoIndex].hbogo, config.tivos[tivoIndex].hulu, config.tivos[tivoIndex].xfinityondemand, config.tivos[tivoIndex].youtube, config.tivos[tivoIndex].epix, config.tivos[tivoIndex].vudu, config.tivos[tivoIndex].plex, config.tivos[tivoIndex].mlbtv, config.tivos[tivoIndex].wwe, config.tivos[tivoIndex].ameba, config.tivos[tivoIndex].toongoggles, config.tivos[tivoIndex].alt, config.tivos[tivoIndex].flixfling, config.tivos[tivoIndex].hsn, config.tivos[tivoIndex].ign, config.tivos[tivoIndex].tastemade, config.tivos[tivoIndex].tubi, config.tivos[tivoIndex].yahoo, config.tivos[tivoIndex].yupptv, config.tivos[tivoIndex].vewd, config.tivos[tivoIndex].baeble, config.tivos[tivoIndex].iheartradio, config.tivos[tivoIndex].pandora];
